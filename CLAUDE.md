...
- `quantizer.rs`: `Rotation` (fixed-seed random orthogonal matrix) + `QuantizedVector` (per-vector int8 affine quantizer). This is a **simplified, documented stand-in** for Google Research's TurboQuant paper (arXiv:2504.19874) — dense O(d²) rotation instead of its structured fast-Hadamard O(d log d) one, uniform quantization instead of its Beta-distribution-optimal one, no QJL residual step. Don't present it as a literal reproduction in docs/comments; it's sized for a single-repo code index, not a billion-vector store.
